    ...
    return `
        <header class="p-4 sm:p-8 pb-4 shrink-0 text-left">
            <div class="flex items-center justify-between mb-6 text-left">
                <button onclick="${state.clientViewMode === 'directory' || state.clientViewMode === 'groups' ? "setApp('launcher')" : "setClientMode('directory')"}" class="flex items-center gap-1 text-slate-400 hover:text-slate-900 transition-colors text-left">
                    <span class="material-icons-outlined text-left">chevron_left</span>
                    <span class="text-xs font-black uppercase tracking-widest hidden sm:block text-left">${state.clientViewMode === 'directory' || state.clientViewMode === 'groups' ? 'Back' : 'Directory'}</span>
                </button>
